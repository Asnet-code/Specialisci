...
import { createSkillAction, toggleSkillActiveAction, updateSkillOrderAction } from "../_actions/skills";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <form className="flex items-center gap-2" action="/admin/skills" method="get">
          <input
            name="q"
        ...
        <form action={createSkillAction} className="grid grid-cols-2 md:grid-cols-5 gap-2">
            ...
            {skills.map((s, idx) => (
              <tr key={s.id} className="border-t border-white/10">
                ...
                <td className="px-3 py-2">
                  <form action={updateSkillOrderAction} className="flex items-center gap-2">
                ...
                <td className="px-3 py-2">
                  <form action={toggleSkillActiveAction}>
                    <input type="hidden" name="id" value={s.id} />
                    <button className="text-xs rounded bg-white/10 border border-white/10 px-2 py-1 hover:bg-white/20">
                      Przełącz aktywność
                    </button>
